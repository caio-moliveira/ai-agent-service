...
crypto = st.text_input(
    "Enter Cryptocurrency Symbol (e.g., BTC, ETH)", value="BTC"
).upper()
currency = st.text_input(
    "Enter Fiat Currency Symbol (e.g., USD, EUR, GBP)", value="USD"
).upper()
...
if st.button("Get Price"):
    with st.spinner("Fetching price..."):
        ...
        fetch_crypto_price = Task(
            config=tasks_config["fetch_crypto_price"], agent=crypto_analyst
        )
